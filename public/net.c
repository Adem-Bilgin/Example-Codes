...
void net_ini(void)
{		// BURAYA "HAL_UART_Transmit(&huart1, (uint8_t)"123456\r\n", 8, 0x1000);" yazmıştı.
	enc28j60_ini();
}
...
uint8_t arp_read(enc28j60_frame_ptr *frame, uint16_t len)
{
	 ...
	 if (len>=sizeof(arp_msg_ptr))
	 {
	   if ((msg->net_tp==ARP_ETH)&&(msg->proto_tp==ARP_IP))
	   {
	     ...
	     {
	    	  sprintf(str1,"\r\nrequest\r\nmac_src %02X:%02X:%02X:%02X:%02X:%02Xrn",
...
void eth_read(enc28j60_frame_ptr *frame, uint16_t len)
{
	 if (len>=sizeof(enc28j60_frame_ptr))
	 {
		 ...
		 else if(frame->type==ETH_IP)
		 {
			...
			frame->addr_dest[3],frame->addr_dest[4],frame->addr_dest[5],len,be16toword(frame->type));
			HAL_UART_Transmit(&huart1,(uint8_t*)str1,strlen(str1),0x1000);
...
void net_pool(void)
{
	...
	net_buf
	enc28j60_packetSend(net_buf, size(net_buf));
	while((len=enc28j60_packetReceive(net_buf,sizeof(net_buf)))>0)
	{
		 HAL_UART_Transmit(&huart1, "Kapali\n", 5, 100);
		eth_read(frame,len);
	}
}
