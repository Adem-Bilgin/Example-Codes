...
{
	SS_SELECT(); // A4 pinini reset yaptı
	SPI_SendByte(op|(addres&ADDR_MASK));
	SPI_SendByte(data);
	SS_DESELECT(); // A4 pinini set yaptı
}
...
void enc28j60_ini(void)
{
	...
	HAL_Delay(2);
	 HAL_UART_Transmit(&huart1, "enc28j60_readOp_start\n", 20, 100);

	//while(!enc28j60_readOp(ENC28J60_READ_CTRL_REG,ESTAT)&ESTAT_CLKRDY);
	 enc28j60_readOp(ENC28J60_READ_CTRL_REG,ESTAT);
	 HAL_UART_Transmit(&huart1, "enc28j60_readOp_end\n", 20, 100);
	...
	enc28j60_writePhy(PHLCON,PHLCON_LACFG2|	PHLCON_LBCFG2|PHLCON_LBCFG1|PHLCON_LBCFG0|
